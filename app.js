...
function queryPointPath(query) {
  ...
  return [...raw.matchAll(/(?:^|\s)п(?:ункт)?\.?\s*(\d+(?:\.\d+)*)/gi)]
    .map((m) => m[1]);
}

function withoutPointPath(query) {
  return String(query || "")
    .replace(/(?:^|\s)п(?:ункт)?\.?\s*\d+(?:\.\d+)*/gi, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function formatArticleCode(article) {
  let code = String(article && article.code || "").trim();
  const tab = String(article && article.tab || "").trim().toLowerCase();

  if (tab === "устав фсб" && /^устав\b/i.test(code)) {
    code = code.replace(/^устав\b/i, "Устав ФСБ");
  }
...
function displayCode(article, query) {
  ...
  const code = formatArticleCode(article);
  ...
  const labels = points.map((point, index) =>
    index === 0 ? `п. ${point}` : `пп. ${point}`
  );

  return `${code} · ${labels.join(" · ")}`;
}

function pointText(text, path) {
  if (!path.length) return String(text || "");
  let result = String(text || "");

  for (let depth = 0; depth < path.length; depth++) {
    const marker = String(path[depth]).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const start = new RegExp(`^\\s*${marker}\\s*[.)]\\s*`, "m").exec(result);
    if (!start) return String(text || "");

    const contentStart = start.index + start[0].length;
    const rest = result.slice(contentStart);
    const next = /^\s*\d+(?:\.\d+)*\s*[.)]\s+/m.exec(rest);
    result = result.slice(start.index, next ? contentStart + next.index : result.length).trim();
  }

  return result;
}

function copyText(article, query) {
  const path = queryPointPath(query);
  const body = [
    displayCode(article, query),
    article.title,
    pointText(article.text, path),
    article.punish
  ].filter(Boolean).join("\n");
...
function hit(a, qraw) {
  ...
  if (nums.length) {
    ...
    const check = art.length ? art : nums;    if (!check.every(t => numHit(num, t, point))) return false;
...
function filtered() {
  ...
  const searchQuery = withoutPointPath(corrected);
  ...
  const textQuery = searchQuery.replace(/(?:^|\s)\d+(?:\.\d+)*(?=\s|$)/g, " ").trim();
  const eligible = scoped.filter(a => hit(a, textQuery));
  const resolved = MemoEngine.resolve(eligible, searchQuery);
  searchPoint = queryPointPath(corrected).join(".") || (resolved ? resolved.point : "");
  const rows = resolved ? resolved.rows : scoped.filter(a => hit(a, searchQuery) || smartHit(a, searchQuery));
  if (mode === "laws") {
...
function render() {
  ...
  $feed.querySelectorAll(".card").forEach(el => {
    ...
    if (btn) btn.onclick = (e) => {
      e.stopPropagation();
      ping();
      const raw = article ? copyText(article, q) : el.innerText.trim();
    ...
    el.onclick = (event) => {
      if (event.target.closest(".article-tools, .point-row")) return;
...
$feed.addEventListener("click", (e) => {
  ...
  if (card && e.detail === 2) {
    ...
    if (article) navigator.clipboard.writeText(copyText(article, $q.value.trim())).catch(()=>{});  }
});
