...
function displayCode(article, query) {
  ...
  if (!points.length) {
    return String(article && article.code || "");
  }
...
function pointText(text, path) {
  ...
  for (let depth = 0; depth < path.length; depth++) {
    const marker = path[depth].replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const suffix = depth === 0 ? "[.)]" : "\\)";
    const start = new RegExp(`^\\s*${marker}\\s*${suffix}\\s*`, "m").exec(result);
...
function copyText(article, query) {
  ...
  return [displayCode(article, query), article.title, pointText(article.text, path), article.punish]
    .filter(Boolean)
    .join("\n");
}
