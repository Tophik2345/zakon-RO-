...
function filtered() {
  ...
  const corrected = searchIndex ? searchIndex.correct(q) : q;  const scoped = db.articles.filter(a => {
...
function closeSidePanels(except) {
  const theme = document.getElementById("themePanel");  if (theme && theme !== except) theme.hidden = true;
